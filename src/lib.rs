...
use ic_cdk::{bitcoin_canister::Network, init, post_upgrade, pre_upgrade};
use std::cell::Cell;
...
/// Pre-upgrade hook.
/// Saves the current state before a canister upgrade.
#[pre_upgrade]
fn pre_upgrade() {
    // Save people state to stable memory
    state::save_people_state();
}

/// Post-upgrade hook.
/// Reinitializes the BitcoinContext with the same logic as `init` and restores state.
#[post_upgrade]
fn upgrade(network: Network) {
    init_upgrade(network);
    
    // Restore people state from stable memory
    state::restore_people_state();
}
...
// Re-export state management functions for external access
pub use state::{
    store_person, get_age_by_name, get_person_by_name, 
    list_all_people, remove_person, get_people_count, clear_all_people
};
